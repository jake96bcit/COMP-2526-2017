...
/**
 * This is Omnivore class.
 * This life form can eat every kind of life form except 
 * another Omnivore.
 * 
 * @author Vinh Le
 * @version 1.0
...
public class Omnivore extends Animal implements Inhabitant {
    ...
    /**
     * Maximum free call and food that needed for
     * the reproduction of Omnivore.
     */
    ...
    /**
     * A Omnivore can give birth if there are: 
     * at least 1 Omnivore neighbours, at
     * least three free neighbouring cell, 
     * and three neighbouring cells with food
     * (Plant, Herbivore and Carnivore). 
     * A Omnivore cannot be born and give birth during the same turn.
     */
    @Override
    public void reproduce() {
        ...
        int randIndex;
        if (omnivoreCount > 0 
                && size > MAX_FREE_OR_FOOD_CELLS - 1
                && foodCount > MAX_FREE_OR_FOOD_CELLS - 1) {
            if (size == 1) {
                randIndex = 0;
            } else {
    ...
    public boolean canBeEattenByCarnivore() {
        int chanceToBeEatten = RandomGenerator.nextNumber(1);
        if (chanceToBeEatten == 0) {
            return true;
        }
        return false;
    }
